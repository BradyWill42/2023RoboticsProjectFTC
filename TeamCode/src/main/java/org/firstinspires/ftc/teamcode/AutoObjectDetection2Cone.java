 ...
 */
@Autonomous(name="ObjectDetection 2Cone", group="Iterative Opmode")
//@Disabled // NOTE: Remove @Disabled to show up in the list of available opModes.
public class AutoObjectDetection2Cone extends LinearOpMode {
...
//                grabber.openGrabber();
//            } else {
                ForwardInches fw = new ForwardInches(-0.7, 38, drivetrain, odometry, simpleOdometry);
                fw.execute();
                StrafeInches st = new StrafeInches(-0.7, -4.5, drivetrain, odometry, simpleOdometry); //6.5 before
                st.execute();
                align();
                alignClose();
...
//            grabber.uptakeSetPos(-1000);
                ...
                TurnAngle ta = new TurnAngle(0.5, -80, drivetrain, odometry, simpleOdometry, gyro);
                ta.execute();
